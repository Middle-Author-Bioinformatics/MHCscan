...
parser.add_argument('-i', type=str, help="attribute for the locus tab (default = ID)", default="ID")

parser.add_argument('-o', type=str, help="output CSV", default="out.csv")

parser.add_argument('--prodigal', type=str, help="input ORFs from Prodigal", const=True, nargs="?")
...
if args.prodigal:
            ...
            if not re.match(r'#', i):
                ...
                if ls[2] == "CDS":
                    try:
                        ...
                        gffDict[contig].append(ID)
                    except IndexError:
                        pass

    numOrfDict = defaultdict(lambda: 'EMPTY')
    for i in gffDict.keys():
        contig = i
        counter = 0
        for j in gffDict[contig]:
            counter += 1
            numericalORF = i + "_" + str(counter)
            numOrfDict[numericalORF] = j
            numOrfDict[j] = numericalORF

    clusterDict = defaultdict(list)
    for i in Dict.keys():
        orf = i
        numericalORF = numOrfDict[orf]
        numOrf = (int(lastItem(numericalORF.split("_"))))
        contig = allButTheLast(numericalORF, "_")
        clusterDict[contig].append(numOrf)
    ...
    for i in clusterDict:
        ...
        for j in clu:
            rep = i + "_" + str(j[0])
            for k in j:
                numericalORF = i + "_" + str(k)
                locus = numOrfDict[numericalORF]
                cluDict[rep].append(locus)
                cluDict2[rep].append(Dict[locus])
                cluDict3[rep].append(hemeDict[locus])
